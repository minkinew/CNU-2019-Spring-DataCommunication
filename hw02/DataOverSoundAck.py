...
def make_sound(byte_stream):
    with noalsaerr():
        freq = list(byte_stream)
        List = []; #List 배열 생성
        List.append(HANDSHAKE_START_HZ) #시작 주파수
        
        #비트스트림으로 변환. 8비트를 비트, 4비트로 나눔
        for i in freq: 
            List.append(((int(i) >> 4)*STEP_HZ)+START_HZ)
            List.append(((int(i) & 0xf)*STEP_HZ)+START_HZ)
        List.append(HANDSHAKE_END_HZ) #끝 주파수

        p = pyaudio.PyAudio() #파이썬 PyAudio 객체를 생성해 소리를 발생시킴
        stream = p.open(format=pyaudio.paFloat32,channels=1,rate=44100,output = True)

        for f in List:
            sample = np.sin(2*np.pi*np.arange(44100*1)*f/44100).astype(np.float32)
            print(f)
            stream.write(sample)
...
def listen_linux(frame_rate=44100, interval=0.1):
    ...
    while True:
        ...
        dom = dominant(frame_rate, chunk) #소리를 주파수로 변환하는 함수(dominant), 소리에 맞게 주파수를 분리 

        if in_packet and match(dom, HANDSHAKE_END_HZ): #끝 주파수를 수신함
            byte_stream = extract_packet(packet) #주파수의 묶음을 데이터로 바꿔줌(extract packet)
            try:
                byte_stream = RSCodec(FEC_BYTES).decode(byte_stream) #RSCodec 함수로 FEC_BYTES 디코딩
                ...
                if byte_stream[:9] == number: #학번이 입력되면
                    ...
                    byte_stream = RSCodec(FEC_BYTES).encode(byte_stream) #RSCodec 함수로 FEC_BYTES 인코딩
